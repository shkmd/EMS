...
export function PermissionApprovalsTable({ scope, actionType }: { scope: string; actionType: "hr" | "manager" }) {
  function actionEndpoint(id: string) {
    return actionType === "hr" ? `/api/permission/${id}/hr-action` : `/api/permission/${id}/manager-action`
  }
  ...
  async function load() {
    const result = await apiFetch<{ requests: ApprovalRow[] }>(`/api/permission?scope=${scope}`)
  ...
  return (
    <Card>
      ...
      <Dialog open={!!actionTarget} onOpenChange={(open) => !open && setActionTarget(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{actionTarget?.action === "APPROVE" ? "Approve" : "Reject"} permission request</DialogTitle>
            <DialogDescription>
              {actionTarget && (
                <>
